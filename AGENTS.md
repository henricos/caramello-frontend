...
Use **apenas** os componentes de UI fornecidos pelo `@ionic/react`. O Ionic garante automaticamente Material Design no Android e iOS no iPhone via *Adaptive Styling*.

- Não crie componentes de UI customizados onde existir equivalente Ionic: use `<IonCard>`, `<IonItem>`, `<IonList>`, `<IonButton>`, etc.
- Evite CSS customizado. Priorize classes utilitárias do Ionic (`ion-padding`, `ion-text-center`, `ion-margin-top`).
- Quando CSS customizado for estritamente necessário, crie-o no arquivo local do componente ou em classes globais em `variables.css`.

## Stack e padrões de código

- Código estritamente em **TypeScript (TSX)**.
- Use **componentes funcionais** e hooks do React (`useState`, `useEffect`, `useContext`). Evite class components.
- Todas as chamadas HTTP centralizadas em serviços dentro de `src/services/` usando `axios`. Nunca faça chamadas diretamente dentro de um componente de UI.

Detalhes de arquitetura: `docs/architecture.md`. Estilo de código: `docs/code_style.md`. Conexão com a API: `docs/python_api_connection.md`.
